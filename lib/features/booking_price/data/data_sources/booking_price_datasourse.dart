...
class BookingPriceDatasourseImpl extends BookingPriceDatasourse{
  ...
  Future<String> distributePrice(PriceDisplay price) async{
    ...
    String msg = success ? "Price updated successfully!" : "There was an error, please try again!";
    return msg;
  }

  @override
  Future<BookingPrice> getAllMonthlyPrice(int productType, String date) async{
    var result = await apiService.get(endPoint: 'api/Pricing/GetMonthlyPrices?productType=$productType&date=$date');
  ...
  Future<PriceLineChart> getLineChart(int productType) async {
    var result = await apiService.get(endPoint: 'api/Pricing/GetLineChart?productType=$productType');
  ...
  Future<PriceDisplay> getPricing(int productType, String date) async{
    var result = await apiService.get(endPoint: 'api/Pricing/GetPricing?date=$date&productType=$productType');
