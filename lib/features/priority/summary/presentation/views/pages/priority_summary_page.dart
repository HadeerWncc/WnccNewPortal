...
import 'package:wncc_portal/features/priority/summary/presentation/views/widgets/priority_symmary_body.dart';
import 'package:wncc_portal/features/user/presentation/manager/cubits/user_cubit/user_cubit.dart';

class PrioritySummaryPage extends StatelessWidget {
  const PrioritySummaryPage({super.key});
        ...
        if (state is UserSuccess) {
          return SafeArea(
            child: Scaffold(
              ...
              drawer: Drawer(
                shape: drawerBorde(),
                child: CustomMenusList(user: state.user, activeTab: 'Summary'),
              ),
              body: const PrioritySymmaryBody(),
            ),
          );
        }
